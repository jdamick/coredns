...
func (h *dnsContext) MakeServers() ([]caddy.Server, error) {
	...
	for addr, group := range groups {
		...
		s, err := createTransport(addr, group)
		if err != nil {
			return nil, err
		}
...
func createTransport(s string, group []*Config) (caddy.Server, error) {
	t, _ := transport.ParseTransport(s)
	if c, ok := transportServers[t]; ok {
		return c.creator(s, group)
	}
	return nil, fmt.Errorf("transport server %s is not registered", s)
}

// DefaultPort is the default port.
const DefaultPort = DNSPort
